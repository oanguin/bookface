    ...
    request(app)
        ...
        .end(function (err, res) {
            ...
            res.body.forEach(element => {
                assert.isEqual(element.password, undefined, "Password should not come back in request");
                assert.isEqual(element.is_admin, undefined, "is_admin should not come back in request");
            });
...
test('POST /user', function (assert) {
    request(app)
        ...
        .end(function (err, res) {
            assert.isNotEqual(res.body, null, "Should not be Equal.");
            /*Password and is_admin should not be returned in request */
            assert.isEqual(res.body.password, undefined, "Password should not come back in request");
            assert.isEqual(res.body.is_admin, undefined, "is_admin should not come back in request");
            /*User id shouuld be returned*/
            assert.isNotEqual(res.body._id, undefined, "Id should come back in the request");
